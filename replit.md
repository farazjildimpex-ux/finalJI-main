...
- **Gmail Email Sending** — Emails are sent via Gmail API (`POST /gmail/v1/users/me/messages/send`) using the same Google OAuth credentials as invoice sync. Builds RFC 2822 MIME multipart messages with real PDF attachments (base64, `Content-Transfer-Encoding: base64`). Requires `gmail.send` scope in addition to `gmail.readonly` — user must re-authorize Google once via Settings → Email. `sendGmailEmail()` in `server/gmailLib.js`.
