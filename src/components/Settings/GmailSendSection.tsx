...
import {
  ...
  ExternalLink, AlertCircle, Loader2, RefreshCw, RotateCcw,
} from 'lucide-react';
...
const GmailSendSection: React.FC = () => {
  ...
  const ConnectSteps = () => (
    <div className="space-y-4">
      ...
      <Step n={2} title={`Sign in with ${status?.email ? status.email : 'office@jildimpex.com'}`}>
        <p>
          {showSwitch
            ? <>Sign in with <strong>office@jildimpex.com</strong> (your Google Workspace account). If another account is already selected, click <em>"Use a different account"</em>.</>
            : <>Sign in with your Google account and click <strong>Allow</strong> on the permissions screen.</>
          }
        </p>
      </Step>
      ...
      <Step n={4} title='Save it as "GOOGLE_REFRESH_TOKEN" in Replit Secrets'>
        <p>Go to <strong>Replit → Secrets</strong> (lock icon in the sidebar). {showSwitch ? <>Delete the old <code className="bg-gray-100 px-1 rounded">GOOGLE_REFRESH_TOKEN</code> and add a new one</> : <>Add a secret named <code className="bg-gray-100 px-1 rounded">GOOGLE_REFRESH_TOKEN</code></>} with the copied value.</p>
      </Step>

      <Step n={5} title="Restart the workflow">
        <p>Go to the <strong>Shell / Workflow</strong> tab and restart the app. Then tap the refresh icon <span className="inline-block align-middle">↻</span> here to confirm it's connected.</p>
      </Step>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              {isReady
                ? `Sending from ${status?.email || 'your Google account'}`
                : 'Connect a Google account to send emails with PDF attachments'}
            </p>
          </div>
        </div>
        ...
        <div className="border-t border-gray-100 px-5 pb-6 pt-5 space-y-5">
          ...
          {isReady && !showSwitch && (
            <>
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-start gap-3 text-sm text-emerald-800">
                ...
                <div>
                  ...
                  <p className="text-xs mt-0.5 text-emerald-700">
                    Emails are sent from <strong>{status?.email}</strong> with real PDF attachments — files arrive directly in the recipient's inbox.
                  </p>
                </div>
              </div>
              <button
              ...
              >
                ...
                Switch to a different account (e.g. office@jildimpex.com)
              </button>
            </>
          )}
          ...
          {isReady && showSwitch && (
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-800 flex items-start gap-2">
              ...
              <div>
                <p className="font-bold mb-0.5">Switching from {status?.email}</p>
                <p>Follow the steps below to connect <strong>office@jildimpex.com</strong> instead. Your existing data is not affected.</p>
              </div>
            </div>
          )}
          ...
          {!status?.configured && (
            <div className="p-3 bg-blue-50 border border-blue-100 rounded-2xl text-xs text-blue-800 flex items-start gap-2">
              ...
              <p>Connect your Google Workspace account (<strong>office@jildimpex.com</strong>) to send contract PDFs and letters directly from the app.</p>
            </div>
          )}

          {/* Show steps when: not configured, needs reauth, or switching */}
          {(!isReady || showSwitch) && <ConnectSteps />}

          {/* Cancel switch */}
          {showSwitch && isReady && (
            <button onClick={() => setShowSwitch(false)} className="text-xs text-gray-400 hover:text-gray-600 underline">
              Cancel — keep {status?.email}
            </button>
          )}
