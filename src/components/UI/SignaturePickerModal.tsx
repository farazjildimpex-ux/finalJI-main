...
const SignaturePickerModal: React.FC<Props> = ({ isOpen, onClose, onSelect }) => {
  ...
  const handleUpload = async () => {
    if (!uploadFile || !uploadName.trim() || !user) {
    ...
    try {
      ...
      setError(e?.message || 'Upload failed. Check storage bucket permissions.');
    } finally {
      setUploading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full sm:max-w-lg bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[92dvh] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100 flex-shrink-0">
          ...
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl text-gray-400 hover:bg-gray-100 transition-colors">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {showUpload ? (
            <div className="border border-dashed border-rose-200 rounded-2xl p-4 bg-rose-50/40 space-y-3">
              ...
              <input
                ...
                className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-400"
              />
              {previewUrl ? (
                <div className="relative inline-block">
                  ...
                  <button
                    type="button"
                    onClick={() => { setPreviewUrl(null); setUploadFile(null); if (fileRef.current) fileRef.current.value = ''; }}
                    className="absolute -top-2 -right-2 w-6 h-6 bg-white border border-gray-200 rounded-full shadow text-gray-400 hover:text-red-500 flex items-center justify-center text-xs"
                  >✕</button>
                </div>
              ) : (
                <button
                ...
                >
                  ...
                  <span className="text-xs font-semibold">Tap to upload PNG / JPG</span>
                </button>
              )}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={uploading}
                  className="flex items-center gap-1.5 px-4 py-2.5 bg-rose-600 hover:bg-rose-500 text-white text-sm font-bold rounded-xl disabled:opacity-60 transition-colors"
                >
                ...
                <button
                  type="button"
                  onClick={() => { setShowUpload(false); setError(''); setUploadName(''); setPreviewUrl(null); setUploadFile(null); }}
                  className="px-4 py-2.5 text-sm font-semibold text-gray-500 hover:text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors"
                >Cancel</button>
              </div>
            </div>
          ) : (
            <button
              ...
              className="w-full flex items-center gap-2.5 px-4 py-3.5 border border-dashed border-rose-200 rounded-2xl text-sm font-semibold text-rose-600 hover:bg-rose-50 transition-colors"
            >
          ...
          ) : sigs.length === 0 && !showUpload ? (
            <div className="text-center py-8">
              <PenLine className="h-10 w-10 text-gray-200 mx-auto mb-3" />
              <p className="text-sm font-semibold text-gray-400">No signatures saved yet</p>
              <p className="text-xs text-gray-300 mt-0.5">Tap "Upload New Signature" above</p>
            </div>
          ) : (
            <div className="space-y-2">
              {sigs.map(sig => (
                <div
                  ...
                  className={`group relative flex items-center gap-3 p-3 rounded-2xl border-2 cursor-pointer transition-all select-none
                ...
                >
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 w-7 h-7 flex items-center justify-center rounded-lg text-gray-300 hover:text-red-500 hover:bg-white transition-all"
                  >
        ...
        {/* Footer */}
        <div className="flex items-center gap-3 px-5 pb-6 pt-3 border-t border-gray-100 flex-shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 px-4 py-3 text-sm font-semibold text-gray-500 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            disabled={!selected}
            className="flex-1 px-4 py-3 text-sm font-bold text-white bg-rose-600 hover:bg-rose-500 rounded-xl disabled:opacity-40 transition-colors"
          >
            Use This Signature
          </button>
        </div>
